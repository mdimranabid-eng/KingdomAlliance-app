  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        const pending = usersData.filter(u => u.isApproved === false).length;
        const males = usersData.filter(u => u.gender === 'male').length;
        const females = usersData.filter(u => u.gender === 'female').length;
        const photoPending = usersData.filter(u => u.photoStatus === 'pending').length;
        ...
        setPhotoPendingCount(photoPending);
      } catch (err) {
